...
public class EntityMergingHelper {
  ...
  public static <T extends Model> T merge(Context context, Class<T> entityClass) {
    return EntityMergingHelper.merge(new HashMap<>(context), entityClass);
  }
  ...
  private static <T extends Model> void processMultiRelation(
    ...
    Collection<Model> collection =
        list.stream()
            ...
            .collect(
                () -> EntityMergingHelper.getDefaultCollection(property.getType()),
                Collection::add,
                Collection::addAll);
  ...
  private static <T> Collection<T> wrap(Collection<T> collection, PropertyType propertyType) {
    if (collection == null) {
      return EntityMergingHelper.getDefaultCollection(propertyType);
    }
    return collection;
  }
