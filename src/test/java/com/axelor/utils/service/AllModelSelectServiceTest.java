...
class AllModelSelectServiceTest extends BaseTest {
    ...
    void testCreateSelectionOfAllModels_NewSelect() {
        List<MetaSelect> existingMetaSelect = metaSelectRepo.all().filter("self.name = ?1", "all.model.reference.select").fetch();
    ...
    void testCreateSelectionOfAllModels_ExistingSelect() {
        ...
        Set<String> metaModelNames = metaModelList.stream()
                .map(MetaModel::getFullName)
                .collect(Collectors.toSet());

        Set<String> metaSelectItemValues = metaSelect.getItems().stream()
